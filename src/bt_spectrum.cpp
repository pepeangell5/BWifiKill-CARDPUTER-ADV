// bt_spectrum.cpp  -  Espectro BT con dos nRF24L01 + sniff BLE
//
// Cambios respecto a la version anterior:
//
// Bugs corregidos:
//   - drawHeader ya no se llama dos veces (antes pintaba "E0" + "E1234"
//     superpuestos sobre el mismo buffer, dejando los digitos OR-eados
//     ilegibles).
//   - onBleSpectrumScanComplete ahora consulta exitingSpectrum antes de
//     tocar el BLEScan (race con btSpectrumExit que nullifica el puntero).
//   - El promedio de RSSI redondea correctamente para enteros negativos.
//   - bleSecondPackets / bleTotalPackets se saturan en vez de hacer rollover.
//   - frameTick muerto eliminado.
//   - min<uint16_t>(viewBlePps, 99) reemplazado por ternario explicito.
//
// Funcionamiento:
//   - bleStrongestEver permanente (vs el del segundo en curso).
//   - Direcciones BLE unicas con timestamp + purga a los 30 s (antes solo
//     crecia hasta 16 y nunca decrecia).
//   - Pico con HOLD (600 ms) + decay lineal rapido (1 unidad / 200 ms),
//     en vez de un decay constante de 1 unidad / 1400 ms.
//   - PLL settle subido a 130 us (datasheet typical).
//   - SAMPLES_PER_CHANNEL bajado a 28 (eran 34; mas no aporta).
//   - Check de begin()+isChipConnected() para detectar radios desconectadas.
//   - LUT precomputado de channel->X (no llamar map() 79 veces/frame).
//
// Animaciones:
//   - Suavizado asimetrico de las barras: subida instantanea, bajada
//     exponencial (displayedSample). Se ve mucho mas pulido que el original.
//   - Pulso BLE solo en el canal de mayor energia entre 37/38/39.
//   - Radio del pulso decae linealmente de 3 a 0 en 600 ms (en vez de
//     desaparecer de golpe).
//   - Grid horizontal punteado al 25/50/75 % del rango.
//   - Marcadores de canales WiFi 1/6/11 (centros) ademas de los BLE adv,
//     con patron distinto para diferenciar.
//   - Quitado el "doble pixel cada 4to canal" sin justificacion.
//   - Flash "hat" sobre la barra durante 3 frames cuando aparece un nuevo
//     emisor (delta grande respecto al frame anterior).
//   - Respiracion sutil del piso cuando no hay actividad (senial vital).
//   - Indicador de scan BLE activo (puntito que parpadea junto a la linea
//     de energia).
//   - Barra de energia total en la columna mas a la derecha del grafico.
