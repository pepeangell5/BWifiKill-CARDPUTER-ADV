...
static const HelpInfo HELP_TABLE[] = {
    ...
    {"Scope para doble nRF.", "UP/DOWN vista BACK", "En Cardputer usa 1 nRF.", true, true},
    ...
    {"Diagnostica el modulo nRF24.", "OK retest BACK salir", "No transmite trafico util.", true, false},
};
...
static void drawPinsPage(const HelpInfo& info) {
    ...
    if (info.usesNrf) {
        char pins[32];
#ifdef BWK_CARDPUTER_ADV
        snprintf(pins, sizeof(pins), "CE%u CS%u SCK%u",
                 AppConfig::NRF1_CE, AppConfig::NRF1_CSN, AppConfig::NRF_SPI_SCK);
        u8g2.drawStr(4, 48, pins);
        snprintf(pins, sizeof(pins), "MISO%u MOSI%u",
                 AppConfig::NRF_SPI_MISO, AppConfig::NRF_SPI_MOSI);
        u8g2.drawStr(4, 59, pins);
#else
